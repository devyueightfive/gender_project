...
1. create an environment `gender_project` with the help of [conda],
   ```
...
2. Create concrete dependencies as `environment.lock.yaml` for the exact reproduction of your
   environment with:
   ```
   conda env export -n gender_project -f environment.lock.yaml
   ```
...
├── src
│   └── gender_project      <- Actual Python package where the main functionality goes.
